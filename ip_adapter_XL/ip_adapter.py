...
class DWConvBlock(nn.Module):
    def __init__(self, dim, drop=0.0):
        super().__init__()
        self.dw = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
...
class Downsample(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.bn = nn.GroupNorm(32, out_ch)
        self.act = nn.ReLU(inplace=True)
...
class HintEncoder(nn.Module):
    def __init__(self, in_ch=3, base=64, base_ch=64, img_size=512, patch_size=16, embed_dim=768):
        super().__init__()
        ...
        self.stage1 = nn.Sequential(
            *[DWConvBlock(64) for _ in range(2)],
            Downsample(64, 256)
        )
        self.stage2 = nn.Sequential(
            *[DWConvBlock(256) for _ in range(2)],
            Downsample(256, 768)
        )
        self.stage3 = nn.Sequential(
            *[DWConvBlock(768) for _ in range(2)],
            Downsample(768, 768)
        )
        ...
        self.blocks = nn.Sequential(*[
            SelfAttBlock(embed_dim, int(embed_dim/64), True)
            for _ in range(4)
        ])
...
class WindowAwareLinearProjection(torch.nn.Module):
    ...
    def __init__(self, cross_attention_dim=1024, clip_embeddings_dim=512, heads=32, train_text=True):
        ...
        if not self.train_text:
            self.cross_attention_dim = cross_attention_dim
            self.box_out = nn.Linear(self.cross_attention_dim, cross_attention_dim)
            self.structure_out = nn.Linear(self.cross_attention_dim, cross_attention_dim)
            self.object_out = nn.Linear(self.cross_attention_dim, cross_attention_dim)
            # self.box_encoder = HintEncoder(in_ch=3, base=64, img_size=512, patch_size=8, embed_dim=768)
            self.object_pos = nn.Embedding(15, 1536)
            ...
            self.condition_self = nn.Sequential(*[
                SelfAttBlock(cross_attention_dim, heads, True)
                for _ in range(2)
            ])
    ...
    def forward(self, text_embeddings, dino_v2, vae, batch=None, training=True, uncondition=False):
        #text_embeds
        ...
        if not self.train_text:
            ...
            box_embeddings = dino_v2(box_mask).last_hidden_state
            # or use:
            # box_embeddings = self.box_encoder(box_mask)
            box_embeddings = self.box_out(box_embeddings)
            
            structure_ref = batch['structure']
            sturcture_ref_embeds = dino_v2(structure_ref).last_hidden_state
            sturcture_ref_embeds = self.structure_out(sturcture_ref_embeds)
            ...
            ref_pos_embeds = self.object_pos(torch.arange(15,device=image_reference_embeds.device).unsqueeze(0).expand(b, -1))
            ...
            image_reference_embeds = image_reference_embeds.reshape(b,n*261,1536)
            image_reference_embeds = self.object_out(image_reference_embeds)

            b, n, c = box_embeddings.shape
            concatenated_tensors = []
            for i in range(b):
                if batch['has_box_mask'][i]==0 and batch['has_appearance'][i]==0 and batch['has_structure'][i]==1:
                    pad = torch.cat([box_embeddings[i], image_reference_embeds[i]], dim=0) * 0.0
                    concatenated_tensors.append(torch.cat([sturcture_ref_embeds[i], pad], dim=0))
                elif batch['has_box_mask'][i]==1 and batch['has_appearance'][i]==1 and batch['has_structure'][i]==0:
                    pad = sturcture_ref_embeds[i] * 0.0
                    concatenated_tensors.append(torch.cat([box_embeddings[i], image_reference_embeds[i], pad], dim=0))
                elif batch['has_box_mask'][i]==1 and batch['has_appearance'][i]==0 and batch['has_structure'][i]==0:
                    pad = torch.cat([sturcture_ref_embeds[i], image_reference_embeds[i]], dim=0) * 0.0
                    concatenated_tensors.append(torch.cat([box_embeddings[i], pad], dim=0))
                elif batch['has_box_mask'][i]==0 and batch['has_appearance'][i]==1 and batch['has_structure'][i]==1:
                    pad = box_embeddings[i] * 0.0
                    concatenated_tensors.append(torch.cat([sturcture_ref_embeds[i], image_reference_embeds[i], pad], dim=0))
                elif batch['has_box_mask'][i]==0 and batch['has_appearance'][i]==1 and batch['has_structure'][i]==0:
                    pad = torch.cat([box_embeddings[i], sturcture_ref_embeds[i]], dim=0) * 0.0
                    concatenated_tensors.append(torch.cat([image_reference_embeds[i], pad], dim=0))
                elif batch['has_box_mask'][i]==0 and batch['has_appearance'][i]==0 and batch['has_structure'][i]==0:
                    pad = torch.cat([box_embeddings[i], sturcture_ref_embeds[i], image_reference_embeds[i]], dim=0) * 0.0
                    concatenated_tensors.append(pad)
        ...
        **kwargs,
        ...
        with torch.inference_mode():
            ...
            if box is None and pil_image is None and appearance is None:
                ...
                if box is not None:
                    box_embeddings = box
                    box_embeddings = box_embeddings.repeat(num_samples, 1, 1)
                    # or
                    ...
                    # box_embeddings = self.text_embedding_projector.box_out(box_embeddings)
                else:
                    box_embeddings=None
                ...
                pad = torch.zeros(num_samples, 5477-contro_embeds.shape[1], c).to(prompt_embeds.device)
                if pad.numel() != 0:
